...
import {
  ...
  GoogleAuthProvider,
  signInWithPopup,
  setPersistence, // << import ตัวช่วยใหม่
  browserSessionPersistence, // << import ตัวช่วยใหม่
} from "firebase/auth";
...
export default function LoginPage() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      // บอก Firebase ให้ใช้หน่วยความจำ session สำหรับการล็อกอินครั้งนี้
      await setPersistence(auth, browserSessionPersistence);
      
      // ดำเนินการล็อกอินด้วย Popup เหมือนเดิม
      await signInWithPopup(auth, provider);
      
      alert("ล็อกอินด้วย Google สำเร็จ! กำลังไปที่หน้าแรก...");
      router.push("/");
    } catch (err: any) {
      setError(err.message);
      ...
      <Card className="w-full max-w-md">
        ...
        <CardFooter className="flex flex-col gap-4">
          <div className="w-full grid grid-cols-2 gap-2">
             <Button onClick={handleSignIn} className="w-full bg-red-600 hover:bg-red-700">
                เข้าสู่ระบบ
             </Button>
